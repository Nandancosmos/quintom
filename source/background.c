...
int background_functions(
  ...
  double Omega_phi, theta_phi, y1_phi, Omega_sig, theta_sig, y1_sig;
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    Omega_sig = pvecback_B[pba->index_bi_Omega_sig_scf];
    theta_sig = pvecback_B[pba->index_bi_theta_sig_scf];
    y1_sig = pvecback_B[pba->index_bi_y_sig_scf];
    pvecback[pba->index_bg_Omega_phi_scf] = Omega_phi; // value of the quintessence field Omega_phi
    pvecback[pba->index_bg_theta_phi_scf] = theta_phi; // value of the quintessence field theta_phi
    pvecback[pba->index_bg_y_phi_scf] = y1_phi; // value of the quintessence field y1_phi
...
int background_indices(
  ...
  class_define_index(pba->index_bg_rho_phi,pba->has_scf,index_bg,1);
  class_define_index(pba->index_bg_p_phi,pba->has_scf,index_bg,1);
  ...
  class_define_index(pba->index_bg_rho_sig,pba->has_scf,index_bg,1);
  class_define_index(pba->index_bg_p_sig,pba->has_scf,index_bg,1);
  class_define_index(pba->index_bg_Omega_sig_scf,pba->has_scf,index_bg,1);
  class_define_index(pba->index_bg_theta_sig_scf,pba->has_scf,index_bg,1);
  class_define_index(pba->index_bg_y_sig_scf,pba->has_scf,index_bg,1);
  ...
  class_define_index(pba->index_bi_Omega_sig_scf,pba->has_scf,index_bi,1);
  class_define_index(pba->index_bi_theta_sig_scf,pba->has_scf,index_bi,1);
  class_define_index(pba->index_bi_y_sig_scf,pba->has_scf,index_bi,1);
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_scf == _TRUE_){
      ...
      printf(" -> w_phi = %1.2e, wished =%1.2e\n",
               -cos(pvecback[pba->index_bg_theta_phi_scf]), -cos(pba->scf_parameters[0]));
...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_){
    ...
    dy[pba->index_bi_Omega_sig_scf] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (pvecback[pba->index_bg_w_tot]+cosh(y[pba->index_bi_theta_sig_scf]));
      
    dy[pba->index_bi_theta_sig_scf] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (-3.*sinh(y[pba->index_bi_theta_sig_scf])-y[pba->index_bi_y_sig_scf]);
      
    dy[pba->index_bi_y_sig_scf] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (1.5*(1.+pvecback[pba->index_bg_w_tot])*y[pba->index_bi_y_sig_scf]
       + y2_sig_scf(pba,y[pba->index_bi_Omega_sig_scf],y[pba->index_bi_theta_sig_scf],y[pba->index_bi_y_sig_scf])*
       exp(0.5*y[pba->index_bi_Omega_sig_scf])*sinh(0.5*y[pba->index_bi_theta_sig_scf]));
  }
...
double y2_phi_scf(struct background *pba,
  ...
  double y_scf = exp(0.5*Omega_phi)*cos_scf(pba,0.5*theta1);
  //General expression for quintessence potentials
  return  scf_alpha0*y_scf + scf_alpha1*y1_phi + scf_alpha2*pow(y1_phi,2.)/y_scf;
}

double y2_sig_scf(struct background *pba,
                  double Omega_sig,
                  double theta2,
                  double y1_sig
                  ) {
    double scf_beta0 = pba->scf_parameters[4];
    double scf_beta1 = pba->scf_parameters[5];
    double scf_beta2 = pba->scf_parameters[6];
    double y_scf_sig = exp(0.5*Omega_sig)*cos_scf(pba,0.5*theta2);
    //General expression for quintessence potentials
    return  scf_beta0*y_scf_sig + scf_beta1*y1_sig + scf_beta2*pow(y1_sig,2.)/y_scf_sig;
}
