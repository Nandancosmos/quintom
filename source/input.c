...
int input_read_parameters(
  ...
  */
  ...
  /* Additional SCF parameters: */
    if (pba->Omega0_scf != 0.){
    /** - Read parameters describing quintessence scalar field potential */
    ...
    pba->theta_phi_ini_scf = (9./10.)*1.e-28*pow(2.*(1-cos(pba->scf_parameters[0]))*(pba->Omega0_cdm+pba->Omega0_b)/(pba->Omega0_g+pba->Omega0_ur),0.5);
    ...
    if (pba->Omega0_scf != 0.){
        /** - Read parameters describing phantom scalar field potential */
        class_call(parser_read_list_of_doubles(pfc,
                                               "scf_parameters",
                                               &(pba->scf_parameters_size),
                                               &(pba->scf_parameters),
                                               ...
                                               errmsg),
                   errmsg,errmsg);
        class_read_int("scf_tuning_index1",pba->scf_tuning_index1);
        class_test(pba->scf_tuning_index1 >= pba->scf_parameters[8],
                   errmsg,
                   "Tuning index scf_tuning_index = %d is larger than the number of entries %d in scf_parameters. Check your .ini file.",pba->scf_tuning_index1,pba->scf_parameters[8]);
        /** - Assign shooting parameter */
        class_read_double("scf_shooting_parameter_phantom",pba->scf_parameters[pba->scf_tuning_index1]);
...
int input_default_params(
  ...
  pba->y_phi_ini_scf = 0.;
  pba->scf_tuning_index1 = 0;
  pba->Omega_sig_ini_scf = 0.;
  pba->theta_sig_ini_scf = 0.;
  pba->y_sig_ini_scf = 0.;
